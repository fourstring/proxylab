...
possible_ip = map(lambda s: s[5:],
                  subprocess.check_output("ip addr | grep -o -E 'inet 192[^/]*|inet 127[^/]*'", shell=True).split("\n")[
                  :-1])
...
class MissingLogException(LabErrorException):
    ...
    def __str__(self):
        return "Your proxy didn't output log in following format:\n" + \
               "\t%s: %s %s %d\n" % (
                   datetime.datetime.utcnow().strftime("%a %d %b %Y %H:%M:%S UTC"), self.client_addr, self.uri,
                   self.size) + \
...
def start_server():
    if strace:
        server = pexpect.spawn("bash -c 'stty -echo -icanon; strace -f -tt ./server 2>>server-strace.log'",
...
def start_proxy(listen_port):
    if strace:
        proxy = pexpect.spawn("bash -c 'strace -f -tt ./proxy %d 2>>proxy-strace.log'" % (listen_port),
                              timeout=DEFAULT_TIMEOUT, logfile=open("proxy.log", "a"))
    else:
        proxy = pexpect.spawn("bash -c './proxy %d 2>&1'" % (listen_port), timeout=DEFAULT_TIMEOUT,
...
def start_client(port):
    ...
    if strace:
        proc = pexpect.spawn(
            "bash -c 'stty -echo -icanon; strace -f -tt ./client %s %d 2>>client-strace.log'" % (ip, port),
            timeout=DEFAULT_TIMEOUT)
    else:
...
def expect_resp_header_only(proc, resp, timeout=-1):
    if proc != None:
        ...
        if line != resp.head_line:
            ...
            raise MismatchException("Client got wrong response line",
...
def build_sc(proxy):
    ...
    try:
        ...
        if outside_proxy == None:
            ...
            client = start_client(outside_proxy)
...
def log_regex(client_addr, uri, size):
    return re.compile("... [0-9]+ ... [0-9]+ [0-9][0-9]:[0-9][0-9]:[0-9][0-9] ...: " + re.escape(
...
def _do_part2_1(server, server_port, client, proxy):
    ...
    for i in xrange(PART2_1_ROUND):
        ...
        cut_point = [random.randint(bigresp.header_end_end + 1, len(bigresp.raw) - 1) for i in
                     xrange(PART2_1_CUT * 2 - 1)]
...
def do_part2_2(server, server_port, client, proxy):
    ...
    for i in xrange(CONCURRENCY):
        ...
        payload += [ConcurrentPayload(servers[client_server_mapping[i]][1],
                                      proxy,
                                      HttpRequest("GET", server_ip, servers[client_server_mapping[i]][0],
                                                  "/" + random_string(random.randint(10, 20))),
                                      HttpResponse(200, random_bytes(random.randint(100, 200))))]
        expect_logs += [{
            "re": log_regex(payload[-1].client.ip, proxy_uri(server_ip, payload[-1].server.port, payload[-1].req.path),
                            len(payload[-1].resp.raw)),
...
candidate_tests = {
    ...
    "all": [test_part1_1, test_part1_2, test_part1_3, test_part1_4, test_part1_5, test_part1_6, test_part1_7,
            test_part2_1, test_part2_2],
}
...
def main():
    ...
    parser.add_argument("-t", action="store_true", dest="ta",
                        help="TA mode, ignore all other args and only print final score")
    ...
    if args.ta:
        ...
        print
        print
        "Score: %d / %d" % (your_score, total_score)
